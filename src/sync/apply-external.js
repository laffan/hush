 ...
 *
 * Historically each provider — Local Sync's watcher + foreground
 * refresh, multi-window's sibling broadcast, and the since-removed
 * Dropbox cursor apply — carried its own copy of the
 ...
 *     caller already holds the lock under the same key, it's left in
 *     place for the caller's own release.
 ...
 * next autosave reasserts it. A most-recent-wins provider can leave
 * it false and apply over a dirty buffer (Versions is the safety
 * net).
 *
