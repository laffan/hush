...
import { rasterizeSelectionToImage, recognizeSelectionHandwriting } from "../selection-raster";
...
export function createSelectionToolbar(state: DrawingState, access?: SelectionRasterAccess): HTMLElement {
  ...
  // buttons and the in-flight dimming for the async recognize path.
  ...
  async function runRecognize(btn: HTMLButtonElement): Promise<void> {
    ...
    try {
      await recognizeSelectionHandwriting({
  ...
  function update() {
    ...
    // a lone image shape, so it's hidden there.
