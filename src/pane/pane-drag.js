...
function showDockOverlay(show) {
  ...
  if (!show) {
    // Reset per-zone proximity classes when the overlay hides so the
    // next drag starts with every zone in its default state instead of
    // inheriting the previous drag's last-known visible set.
...
/** Show each drop zone only when the cursor is within `NEAR_PX` of its
 *  edge. Called from the drag pointermove so the four blue trapezoids
 *  surface lazily as the user approaches an edge instead of bracketing
 *  the entire drag with all four lit up. */
...
export function setupPaneResize(pane, deps) {
  ...
  for (const handle of pane.el.querySelectorAll(".fp-resize")) {
    handle.addEventListener("pointerdown", (e) => {
      ...
      // synchronous layouts per second and choke the main thread.
