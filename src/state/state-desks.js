 ...
 *  (to recover trees that were broken before this fix landed).
...
export async function migrateLegacyTreeIfNeeded(state) {
  ...
  if (tree.some((n) => n.type === "desk")) {
    ...
    // tree desk — boot-time recovery for installs that accumulated
    // orphans (the removed sync layer used to leak these).
