...
| 11 | this branch | Two-part desk-content arrival fix. (a) `looksLikeUnwrappedDeskSkeleton` no longer requires an `Inbox` sub-child — a desk whose Mac-side layout puts a project at the root (no Inbox) is just as valid an unwrapped skeleton, and the Inbox gate was leaving the orphan top-level folder unabsorbed (empty new desk + invisible orphan with all the content). (b) `ensureDesksTreeSpecials` skips top-level "stragglers" whose name matches a known desk in `settings.desks` (or any desk in the tree) — defense in depth so a future orphan can't be folded into the active desk and trigger `reconcileSync` to rewrite Dropbox to `/Personal/<DeskName>/...`. |
