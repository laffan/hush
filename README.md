...
**Outlines** are nested checklists with a frame round them. Put `outline: true` in a document's frontmatter and every nested checklist in that file becomes one: a subtle border, text a couple of pixels smaller than the prose and set in the app's UI font rather than the style's, completed items struck through and faded back, and the first unfinished item bold in the style's header colour. A minimal footer keeps the tally and carries two toggles — **hide completed items**, and **pin to bottom**, which lifts the outline out of the text and docks it to the bottom of the window or frame so it stays in view however far past it you write. **Convert to Outline** in the command palette turns the list under the cursor into one: a checkbox on every item that hasn't got one, and the frontmatter switch set. The same command in a notebook folds a selected flowchart into a single outline shape — the chart's tree read out as a nested checklist, in the order the chart was laid out. Nothing is stored but the markdown, so an outline is the same outline on either surface: drag one into a document and it arrives as the checklist it is.
