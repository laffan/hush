...
- **Set word count limit** caps a single document. Pick a number from the command palette and the doc stops taking new words when it reaches it — a paste that overshoots lands trimmed to the words that still fit, and from ten words out a red countdown sits at the bottom of the window. The cap belongs to the document, so a floating pane, a stack column or Zen Focus showing that doc holds the same line. Run the command again to change the number; **Clear word count limit** lifts it.
