...
- **Proofread PDF** turns the open PDF into a `<name>-Proof` notebook: every page rendered at twice actual size on a blank background, stacked in one column, on a locked layer with a drawing layer over it. From there it's an ordinary notebook — ink, text, drag boxes, and especially Splits and Grabs, which is how you open space between paragraphs or move a section to where it belongs. A page rail rides the edge of the shelf: not a list of the pages the PDF had but a live minimap of the document as it stands, so a split shows up there as two pieces, each the shape the cut left it. Drag its left edge to make it bigger.
