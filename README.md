...
- **Set word count limit** caps a single document. Pick a number from the command palette — never fewer words than the document already holds — and the doc stops taking new words when it reaches it; a paste that overshoots lands trimmed to the words that still fit. The cap shows itself through the word count rather than in an indicator of its own: `320 / 500 words`, red for the last ten, `500 - limit reached` at the cap — and those last ten put the count on screen whether or not you keep it switched on. The cap belongs to the document, so a floating pane, a stack column or Zen Focus showing that doc holds the same line. Run the command again to change the number; **Clear word count limit** lifts it.
