...
**The editor** is Markdown with inline rendering — headings, bold/italic, links, tables, images, footnotes, callouts, YAML properties, and `[[wikilinks]]` between notes. Sixteen color themes across light, dark, and sepia, and **Styles** that bundle theme + font + colors into presets you can switch, share, and import as JSON. Every colour in the style editor opens an in-app picker — a rainbow hue slider, an opacity slider and a saturation/brightness field — so the live preview stays visible while you tune. The opacity slider is there on every one: the palette, the caret and its glow, the line indicator's custom colour, gradient nodes, WebGL and caret effects, and post-processing colours. A style's **Cursor** section sets the caret's shape — the system caret, a block, an underline or a 3px line — and its colour for the light and dark appearances side by side. A custom caret also has an **Idle Animation** — the usual blink, a slow pulse between 20% and full opacity, or none — and can carry a **glow** in a colour of its own, switched on separately for the light and dark appearances, with an intensity slider that sets how far the halo spreads. The preview pane plays the idle animation the editor will.
...
- **Focus Mode** (`⌘S`) dims everything but the current sentence — the sidebars, the word count and the iPad touch-mode buttons fade to the same level, and headings, highlights and the YOUAREHERE tag go monochrome everywhere but the sentence you are in; **Typewriter Mode** pins the cursor to a fixed line.
...
- **Set word count limit** caps a single document. Pick a number from the command palette — never fewer words than the document already holds — and the document takes no new word once it reaches that number. The word you are in the middle of is still yours to finish — a half-typed word already counts as a word, so the cap is often reached mid-word — but the space after it is refused, and so is anything else that would start another one. Below the cap a paste that overshoots lands trimmed to the words that still fit; at it, a paste lands nothing. Deleting and rewriting are untouched: the wall is on adding, not on editing. The cap shows itself through the word count rather than in an indicator of its own: `320 / 500 words`, red for the last ten, `500 - limit reached` at the cap — and those last ten put the count on screen whether or not you keep it switched on. The cap belongs to the document, so a floating pane, a stack column or Zen Focus showing that doc holds the same line. Run the command again to change the number; **Clear word count limit** lifts it.
...
- Sentence-level navigation and editing shortcuts: jump, select, and move whole sentences from the keyboard. **⌘⌫** deletes the sentence the cursor is in — the one ⌘L would select, so at the end of a paragraph it takes the paragraph's last sentence — along with the space it leaves behind.

**Outlines** are nested checklists with a frame round them. Put `outline: true` in a document's frontmatter and every nested checklist in that file becomes one: a subtle border, text a couple of pixels smaller than the prose and set in the app's UI font rather than the style's, completed items struck through and faded back, and the first unfinished item bold in the style's header colour. A minimal footer carries two toggles — **hide completed items**, and **pin to bottom**, which lifts the outline out of the text and docks it to the bottom of the window or frame so it stays in view however far past it you write; drag the panel's top edge to set how much of the frame it takes. In Zen Focus an outline folds away for the duration — a pinned one comes back as the one item you are on, at the top of the screen. Option-↑ / Option-↓ move an item among its siblings and take everything nested under it along. **Convert to Outline** in the command palette turns the list under the cursor into one: a checkbox on every item that hasn't got one, and the frontmatter switch set. **Convert to List** is the way back — it takes the boxes off the outline under the cursor, and drops the frontmatter switch when that was the document's last outline. **Outline mode** is the switch on its own, for a document whose checklists are already written. The same command in a notebook takes either of two selections: a flowchart folds into a single outline shape — the chart's tree read out as a nested checklist, in the order the chart was laid out — and a text shape that is a markdown list becomes an outline where it stands, checkboxes and frame and all. Nothing is stored but the markdown, so an outline is the same outline on either surface: drag one into a document and it arrives as the checklist it is.
...
Notebooks are infinite pan-and-zoom canvases for visual thinking: text shapes (with Markdown), images, freehand drawing with pressure-sensitive brushes, drag-area containers, layers, and camera bookmarks. Text shapes connect into **flowcharts** with directed arrows, and any flowchart folds into an **outline** — one shape holding the whole tree as a nested checklist, with its own frame, checkboxes and footer; a **shelf** panel lists every shape as an outline; a **pocket** stashes shapes off to the side; **Brainstorm mode** captures rapid-fire ideas.

The wheel scrolls the canvas the way it scrolls a document, both ways; hold shift to pin a swipe to the axis you were already travelling on until you let go, and ⌘-wheel (or a trackpad pinch) to zoom. Locking a layer takes its contents out of reach of the pointer entirely — they still draw, they just stop being something you can pick up by accident. On iPad, only the Apple Pencil draws — fingers pan, pinch, tap to undo, and drag out a selection box without leaving the brush. Handwritten strokes can be **recognized into text** on-device (ML Kit on iPad; Apple Vision for images anywhere). Any selection can be **rasterized** into a single image that keeps tracking light/dark appearance.
...
**Start timer** in the command palette opens a sheet in Courier's style: what you're working on, **Timer** or **Alarm** — hours and minutes from now, or a time of day (the next time the clock reads it, today or tomorrow) — then a timeline laid out from the current time that shows when each break falls and when you'll be done, and redraws as you change anything, and under it how often to take a break (never, or every 25, 30, 45, 60 or 90 minutes). Everything sits centred on the sheet. Start, and the timer sits at the bottom of the files sidebar, floating over the file list just above Add and Settings: the task, a thin progress bar under it, then the time to go, the time to the next break and the finish time, in whole minutes. Double-click the task to reword it; the clock carries on untouched. With the sidebar closed the timer shrinks to a small ring in the lower-left corner that fills as the session runs — hover it for the minutes to the next break, click it to open the sidebar — and it fades with everything else in Focus mode. A toast says when a break comes up and when time is up. For an alarm, the last ten minutes turn the minutes to go red, and the ring shows them without a hover.

There is one timer at a time, the same one on every desk and in every window, and it keeps counting while Hush is closed. **Delete timer** in the command palette takes it away; once a timer has finished, **Start timer** replaces it.
...
- **The Inbox** is where new files land, and its rows say so: each one carries the time it was last edited under its name, in red once it has been sitting there a week.
- **Folders and projects** organize files. Opening a file from anywhere — the sidebar, the command palette, Recent Files, YOU ARE HERE, a wikilink — unfolds the folders and projects above it in the sidebar, so you can always see where it lives; a project's documents concatenate into one continuous editor buffer with separators, so a long piece can live across many files. **Show numbers** in a project's menu numbers the documents that make up that text in the sidebar; notes, notebooks, stacks and the project's PDFs sit outside it and stay unnumbered. Projects convert to and from single tabbed documents, and a doc can be **split at its headings** into a project.
...
- **Floating panes** open any file as a reference window over your work: dock them to an edge (a docked document keeps the editor's column width, centred in the pane), pin them across documents, attach them to canvas positions, or embed them inline under a wikilink. A **Gutter** docks a notebook beside a document, scroll-locked to it, for marginalia.
...
- **PDF viewer**: fast, annotation-aware (Zotero highlights and ink render on the pages), with an annotation shelf, named colored **bookmarks** that work as deep links from your notes, a **folded view** that collapses a paper to just its annotated regions, and a gallery **shelf** view of any PDFs folder with full-text-ish search over titles, bookmarks, and highlight text. Double-click anywhere on a page for a **clip** — a bookmark that points at that exact spot, drawn as a dot with a dashed line out to a clip tab on the page's edge; ⌘-drag the dot to move it, and the line and tab follow.
- **Create Proofread Notebook** (command palette, or a PDF's row menu in the sidebar) turns a PDF into a `<name>-Proof` notebook. It asks which pages first — all of them by default, or `1-3, 5-8` — then renders each one at twice actual size on a blank background, stacked in one column, on a locked layer with a drawing layer over it. Whatever you had already highlighted or drawn on the PDF comes with it, printed into the pages. The highlighter arrives set to **blend**, so a mark over a page darkens the words rather than laying a slab over them — a checkbox in the brush flyout, on by default here and off in an ordinary notebook, where a dark canvas would swallow the ink. From there it's an ordinary notebook — ink, text, drag boxes, and especially Splits and Grabs, which is how you open space between paragraphs or move a section to where it belongs. A page rail rides the edge of the shelf: not a list of the pages the PDF had but a live minimap of the document as it stands, with your own strokes and notes sketched on top, so a split shows up there as two pieces, each the shape the cut left it. Click anywhere on it — a page or the gap between two — to glide down to it, without disturbing where you've scrolled to sideways. Drag its left edge to make it bigger, or hide it altogether from the toggle beside the canvas-rotation and background buttons. On iPad a weighted scroll wheel sits in the opposite corner, since the hardware has nothing that travels a long document well: drag its face to move through the document, flick it and it keeps spinning, touch it again to stop it dead. Its foot is a grip — drag it and the wheel moves, let go and it stays. Its cap is a magnifier: tap it and the same drag drives zoom instead of position, with the whole control outlined in red for as long as it does. Both wheel and rail travel with the proof into a floating pane or a stack column, where they measure against the pane's own edges rather than the screen's — so you can keep a proof open beside the draft you're writing from it.
- **Save to Desk** sits beside Export whenever the format is PDF, in both the document and notebook export modals: the rendered PDF lands in the desk's PDFs folder instead of on disk, ready to read, annotate, or proofread without a round trip through Finder.
