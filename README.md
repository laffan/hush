...
- **File views** *(experimental)* change what the sidebar's file manager *is*. Beyond the ordinary **List**, a desk can keep its files as a **Desktop** or a **Drone** view, chosen from the command palette (`Files: Desktop view`, `Files: Drone view`, `Files: List view`) and remembered per desk — a reference desk can stay a list while a drafting desk stays a desktop. Both are one canvas in place of the tree: drag to pan, ⌘-wheel or pinch to zoom, and a small chip in the corner cycles between the three.
  - **Desktop** is the desk as a desktop. Folders look like folders, files look like pages, and you can drag any of them anywhere; where you leave them is remembered for this screen. Double-click a folder and it does *not* open a window — its contents bloom into a ring around it, joined by hairlines, with the rest of the desk dropping behind a scrim. A folder inside that ring blooms into its own arc facing outward, so a path through the tree reads as one growing diagram. A single click opens a file, exactly as in the list; Escape closes every ring.
  - **Drone** is the desk as a place seen from above: a squarified treemap extruded into a top-down 3D model. Each folder is a plot whose area is what it holds, each file a building standing on it, nesting is elevation, height is the file's size, and a roof brightens the more recently the file was edited. Row colours and the file open in the editor carry through. Tap a building to open it, tap a plot's name to fly down into it (the breadcrumb flies back out), and drag the compass in the corner to orbit.
  - Both views draw the desk's own tree. Renaming, flagging, colouring, deleting and reordering stay in the List view, along with the Flagged section and Local Folder mounts.
