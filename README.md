...
- **iPad**: full app with Apple Pencil support, Files-app integration, real multi-window (Split View / Stage Manager), and touch-mode pills for keyboard-free use. A window that fills the screen hides the status bar (Settings → Hide status bar in full screen); a windowed one keeps it, since that's where iPadOS puts the window controls. iPhone gets a phone-tuned layout.
