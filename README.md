...
- **Send Selected** — opens a fuzzy file picker filtered to the current type (Docs in doc mode, Notebooks in notebook mode, current file excluded) and appends the selection to whichever target you pick. If the target happens to be open the live view refreshes; otherwise it's an on-disk write.
