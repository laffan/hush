...
**Doc Images** — Drag any image file from the desktop into a document to embed it at the drop point. The editor renders the image inline, scaled to the editor column width and capped at 3/4 of the window height. Images are referenced with standard markdown syntax (`![alt](brown-cow.png)`), with an optional caption after a pipe (`![alt | caption](brown-cow.png)`). Every dropped image is stored in a top-level **Images** folder just above Trash — hovering over an image in that folder previews it in a tooltip; clicking opens a full-size modal. Images can be renamed from the Files panel; every reference across every doc is rewritten automatically. Cmd-drag carries an image reference between documents and panes. Exporting a doc that contains images produces a folder with `text.md` plus an `images/` subdirectory.
