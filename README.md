...
The wheel scrolls the canvas the way it scrolls a document, both ways; hold shift to pin a swipe to one axis until you let go, and ⌘-wheel (or a trackpad pinch) to zoom. On iPad, only the Apple Pencil draws — fingers pan, pinch, tap to undo, and drag out a selection box without leaving the brush. Handwritten strokes can be **recognized into text** on-device (ML Kit on iPad; Apple Vision for images anywhere). Any selection can be **rasterized** into a single image that keeps tracking light/dark appearance.
