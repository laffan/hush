...
- **Set word count limit** caps a single document. Pick a number from the command palette — never fewer words than the document already holds — and the document takes nothing more once it reaches that number: not a word, not a letter, not a space. Below the cap a paste that overshoots lands trimmed to the words that still fit; at it, a paste lands nothing. Deleting and rewriting are untouched — the wall is on adding, not on editing — so a word the wall lands in the middle of (a half-typed word counts as a word) is finished by selecting it and retyping it whole. The cap shows itself through the word count rather than in an indicator of its own: `320 / 500 words`, red for the last ten, `500 - limit reached` at the cap — and those last ten put the count on screen whether or not you keep it switched on. The cap belongs to the document, so a floating pane, a stack column or Zen Focus showing that doc holds the same line. Run the command again to change the number; **Clear word count limit** lifts it.
