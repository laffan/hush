...
- **Create Proofread Notebook** (command palette, or a PDF's row menu in the sidebar) turns a PDF into a `<name>-Proof` notebook. It asks which pages first — all of them by default, or `1-3, 5-8` — then renders each one at twice actual size on a blank background, stacked in one column, on a locked layer with a drawing layer over it. From there it's an ordinary notebook — ink, text, drag boxes, and especially Splits and Grabs, which is how you open space between paragraphs or move a section to where it belongs. A page rail rides the edge of the shelf: not a list of the pages the PDF had but a live minimap of the document as it stands, with your own strokes and notes sketched on top, so a split shows up there as two pieces, each the shape the cut left it. Click anywhere on it — a page or the gap between two — to glide down to it, without disturbing where you've scrolled to sideways. Drag its left edge to make it bigger. On iPad a weighted scroll wheel sits in the opposite corner, since the hardware has nothing that travels a long document well: drag its face to run down the page, flick it and it keeps spinning, touch it again to stop it dead.
