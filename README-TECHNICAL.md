...
Desktop-only direct-filesystem folder mounting. The sidebar's footer **Add (+)** popover carries a **Local Folder** entry (`add-popup.js`) that opens the OS folder picker via `addLocalSyncFolder()` in `sync/local-sync.js` and registers the result. Each mounted folder appears in the files panel as a top-level node with its own icon — a circle bisected by a horizontal line — distinct from regular folders and from the Dropbox root. The hamburger menu on each mounted-folder row (built in `files-panel-local-sync.js`) surfaces a single **Unlink Folder** entry.
