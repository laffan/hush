...
- **`html-to-markdown.js`** — pure converter. Walks the DOM produced by `DOMParser` and emits Hush-flavoured markdown. Handles `<h1>`–`<h6>`, `<p>`, `<ul>`/`<ol>`/`<li>`, `<blockquote>`, `<a>`, `<br>`, `<hr>`, `<code>`/`<pre>`, plus the inline emphasis tags (`<b>`, `<strong>`, `<i>`, `<em>`, `<s>`, `<del>`, `<mark>`). Google Docs's quirks are special-cased on the way in: the `<b id="docs-internal-guid-…">` envelope is unwrapped before block walking (otherwise every block would render as bold), inline-styled `<span style="font-weight:700">` / `font-style:italic` / `text-decoration:line-through` are detected via small style regexes alongside the standard tags, `<span style="background-color:…">` (non-white / non-transparent) becomes `==highlight==`, and `https://www.google.com/url?q=…` redirect links are peeled back to the real destination via `URL.searchParams`. Returns `null` when the input doesn't contain any of `RICH_TAG_RE`'s tokens, so plain-text pastes fall through to CodeMirror's default path untouched. **Lists are coalesced before rendering**: Google Docs emits each list item as its own sibling `<ul>`/`<ol>` block with `aria-level` on the `<li>` indicating visual depth (rather than the standard nested-`<ul>`-inside-`<li>` pattern), so `blockListGroup` walks every consecutive sibling list at the same DOM level, collects each `<li>` with its depth (read from `aria-level` first, falling back to ancestor-list count for standard markup), renormalises the depth values to a contiguous 0..N range so visually-jumped levels collapse cleanly, then emits indented markdown with per-depth ordered-list counters that pop when the depth decreases (so a later sibling group restarts numbering correctly). A `data-hush-consumed` flag on already-processed lists keeps the second-and-later sibling from re-entering the group at the top level.
