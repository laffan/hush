...
- **Inline-pane arrow (`pane/pane-inline.js`)** — `WikilinkWidget` carries a `▾` button next to every resolved link in the main doc editor (`createWikilinkPlugin(state, { onInlinePaneRequest })` enables the arrow; the pane editor's `createBaseExtensions` path opts out so embedded panes don't recursively render arrows). Click → `openInlinePaneForWikilink(state, { title, occurrence })`, which resolves the target and calls `createPane(..., { inline: { anchorTitle, occurrence, height } })`. The pane is parked off-screen inside `#pane-container` long enough for `loadPaneContent` to mount CodeMirror against a live DOM, then the **inline-pane plugin** (`createInlinePanePlugin`, registered in `editor.js`) reparents `pane.el` into a CM block-widget host placed at the end of the line containing the Nth `[[Title]]` match. The widget's `eq()` is keyed on pane id so CM reuses the same host DOM across rebuilds and never tears the embedded editor down. `_collectPaneMetrics` in `pane-manager.js` skips `pane.inline` so the editor column doesn't lateral-shift; resizing routes through `syncInlinePaneSize` → `view.requestMeasure()`; dragging the titlebar past a 4 px threshold runs `detachInlinePane` (clears `pane.inline`, reparents `pane.el` to `#pane-container` at the cursor, re-emits `panes-changed` so the host is dropped), at which point the floating-pane code path takes over and make-space lateral shift kicks in normally. Inline anchors persist via `panes.json` as `{ anchorTitle, occurrence, height }` and re-mount on the next doc open. Stacks were added to `getLinkableNotes`'s eligible types so wikilinks (and inline panes) can target them in addition to docs + notebooks.
...
The Sync settings tab is split into two sub-tabs (**Dropbox Sync** / **Google Sync**) so each provider's chrome stays focused. Local Sync used to live here as a third sub-tab; it now mounts from the sidebar's Add (+) menu ("Local Folder") and folder management (unlink) lives on each Local Sync row's hamburger dropdown. The active sub-tab is module-local in `settings/settings-tabs-sync.js` (a `_activeSubTab` variable plus `getSyncSubTab` / `setSyncSubTab` exports). The outer `renderSyncTab(settings)` paints a 2-button nav at the top of the panel and delegates the body to whichever sub-renderer is active (`renderDropboxSubTab`, `renderGoogleSubTab`). Bindings live in `settings/settings-sync-tab.js`: a single `bindSyncTab` wires the sub-tab nav clicks (which call `setSyncSubTab(id)` then `render()`) plus the Dropbox handlers, and a sibling `bindGoogleSubTab` wires Google's credential-form / connect / disconnect / test-connection / clear-log buttons.
...
**Frontend modal** (`sidebar/doc-export-modal.js`). The sidebar's Export button (and the `export-current-file` event) opens this modal whenever a doc or project is the active surface; notebooks still take the existing notebook-export modal. The modal is two columns — controls on the left, a live preview on the right. For PDF the preview *is* the real Typst render (debounced ~350 ms on any option change) painted into the pane via `pdf.js` (`paintPdfPreview` → `getPdfjs` from `pdf/pdf-viewer.js`); the rendered bytes are cached keyed on the render-relevant options so pressing Export reuses them instead of rendering twice. Markdown shows the joined buffer as text and defers to the established `exportCurrentFile` path. The PDF panel pulls its style list from the `list_doc_styles` Tauri command so adding a style is a Rust-only change. Layout toggles (`Strip comments`, `Strip flags`, `Include tabs`, `Number headings`, `Page numbers`) ride a generic `data-choice` wiring so adding another toggle is HTML-only; the **Include bibliography** toggle only appears when Zotero is configured *and* the doc contains at least one `[@key]` reference (cheap regex sniff). References are loaded whenever the doc cites anything (not just when the bibliography is on) so inline citations can be formatted either way (see `markdown.rs`). Render hands the bytes to the platform-appropriate sink — `navigator.share` on iOS, `save` dialog + `write_binary_file` on desktop.
...
- **`styles.rs`** — `Style` registry plus the `wrap()` composer. A style is a Typst preamble string plus an id and name; `wrap()` prepends the preamble, optionally appends `#set page(numbering: "1")` and `#set heading(numbering: "1.1")`, maps the modal's line-spacing toggle to a `#set par(leading: …)`, then the converted body, then the bibliography directive when references are in play — forcing `#set page(columns: 1)` before it so the reference list reads full-width even under a multi-column style. Two styles ship: *Formal* (Libertinus Serif, US-letter, white background) and *Article (2 Column)* (two-column, sans-serif Karla); the registry table at the bottom of the file is the one place to add another.
...
- **`commands/local_sync.rs`** — `local_sync_add` / `_remove` / `_list` / `_read_dir` / `_read_file` / `_write_file` / `_read_file_bytes` / `_write_file_bytes`. `local_sync_add` takes an optional `desk_id` parameter so the mount is stamped with the currently active desk on creation. The byte variants surface image binaries that live next to a Local Sync `.md` file (`_write_file_bytes` auto-suffixes on collision, mirroring `ImageManager::unique_filename`). Includes the local-only `find_local_sync_folder` helper and a small `uuid_like()` ID generator
