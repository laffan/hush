...
**Desktop / File mode + pane indicators** (`sidebar/files-panel-pane-indicators.js`). A doc or notebook with at least one floating pane is in "desktop" mode; the same file with no panes (or hidden panes) is in "file" mode. The files panel paints a per-row strip of 7 × 9 px squares under the filename — one per pane the file owns — by appending a `<span class="tree-pane-indicators">` inside a flex-column wrapper around the normal `tree-item-row`. The strip is built by `paneIndicatorsFor(item, state)` which calls `getPanesForContext(contextIdForFile(item.fileId, item.type))` on the live pane Map. `attachPaneIndicatorTooltip(listContainer)` adds delegated `mousemove`/`mouseleave` handlers that surface a small fixed-position tooltip with the pane's `fileName` while the user `cmd`-hovers a cell (event-level `metaKey` check — no global modifier listener). The strip rerenders on `panes-changed` / `panes-hidden-changed`, both emitted from `pane-manager.js#notifyLayoutChange` / `state-panes.js`. **Hide / Show panes** (command palette) toggle `settings.panesHiddenByContext[ctx]`: a truthy entry hides every pane participating in that context (owner-match or pinned-into) and applies a CSS class that collapses the indicator strip to a 2 px high, 50 % opacity dim bar so the row still reads as a desktop. **Clear panes** drops every pane in the active context via `clearPanesForContext(ctx)`. **Copy panes to Document** / **Switch panes to Document** open a picker (active desk scope, current file excluded), then run `copyPanesBetweenContexts(sourceCtx, targetCtx)` to re-instantiate each pane under the target's `ownerContext` (preserving size, position, attach, pin, collapsed). The Switch variant additionally clears the source context, force-clears the target's hidden flag, and opens the target so the panes ride along. Per-context hidden flags are intentionally local-only — they're not part of the cross-device pane sync payload.
