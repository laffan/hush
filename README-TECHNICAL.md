...
**Pane overlay + minimap.** Notebook desktop thumbnails project two extra layers on top of the snapshot canvas. `renderNotebookSnapshotThumbnail` was extended to return its computed camera + content bounds (or `computeSnapshotCamera` for callers that already have bounds); `desktop-thumbnail.js` stashes the camera on the `<canvas>` after each repaint so subsequent overlay passes can transform world coordinates without re-decoding the envelope. (1) **Pane rectangles** — every persisted pane whose `ownerContext` matches the desktop file (and every pinned pane) gets a translucent grey rectangle. Canvas-attached panes use `_canvasX`/`_canvasY` projected through the snapshot camera so the rect lands on top of its anchor shape; free-floating panes fall back to screen-pixel scaling. (2) **Viewport minimap** — when the desktop file is the currently-open notebook, a blue rectangle echoes the live camera's world viewport, refreshed on every animation frame. Pointerdown + drag inside the body pans the live canvas so the clicked world point lands at the centre of the live viewport (the open-on-click action is suppressed via a scope-local flag while a minimap pan is in flight). The body's `cursor: crosshair` advertises the affordance. (Phase 2 of the Desks rollout will extract this minimap projection into a standalone widget so the desktop thumbnail becomes a pure preview.)
