...
**Desks sync.** `.hush/desks.json` carries the synced part of the data model — `useDesks`, the `desks` list, and `desksMeta` — through the existing meta-sync pipeline (`sync/desks-sync.js`). The applier handles tree wrap / unwrap on the receiving device when `useDesks` flips. Dropbox layout migration runs from the toggling device only: `sync/desks-migration.js` enumerates `synced_files`, enqueues `rename` ops through the op log to prefix every path with `<DeskName>/` (or strip it on toggle off), and updates `relative_path` via `rename_sync_file`. Inbox / Images / Trash subfolder contents merge into the global specials when desks turn off; non-special children of a desk become a top-level folder named after the desk (or hoist to root when there's only one desk). Receiving devices don't enqueue any Dropbox writes — the cursor delta picks up the source's moves naturally.

**Desks** (`state/state-desks.js`) sit one level above everything else when `settings.useDesks` is true. The top-level tree then holds `type: "desk"` nodes; each desk carries its own per-desk Inbox / Images / Trash addressed as `__inbox__:<deskId>`, `__images__:<deskId>`, `__trash__:<deskId>`. Active-desk routing goes through `state.getInboxId()` / `getImagesId()` / `getTrashId()` (delegated to `state-desks.js#activeSpecialId`); `state.isSpecialNodeId` and `state.isInTrash` match across every desk's namespaced ids by prefix. `enableDesks(state, name)` wraps the existing flat tree under one new desk (default "Personal") and re-namespaces the legacy specials into it; `disableDesks(state)` reverses the wrap, merging every desk's specials into a single global Inbox / Images / Trash and either hoisting the lone desk's content to the top level (when there's only one desk) or preserving each desk's non-special content as a top-level folder named after the desk. Per-desk synced metadata (active style, desktop slot, persisted panes) lives in `settings.desksMeta[deskId]`; the active desk id (`settings.activeDeskId`) is intentionally local-only — each device picks its own. Tree traversal helpers in `state/tree-helpers.js` know about the desk node type via prefix-matching the special-id checks (`pinSpecialsInList`, `enforceSpecialPositions`, `sanitizeDescendants`, `normalizeProjectChildren`).
