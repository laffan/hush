...
**Filename rule for linked docs.** `state/state-naming.js#maybeRenameFromFirstLine` and `maybeRenameFileFromContent` both short-circuit when `state.settings.googleDocLinks[fileId]` is set — the Google Doc title is the source of truth for the filename, and the first-line auto-rename would otherwise overwrite it with the first content line (or the leading tab marker). `deriveName` also walks past `---Tab name---` markers and blank lines so even non-linked docs whose first content line follows a marker pick up a sensible filename.
