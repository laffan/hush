...
**Desks sync.** `.hush/desks.json` carries the synced part of the data model — the `desks` list and `desksMeta` — through the existing meta-sync pipeline (`sync/desks-sync.js`). `applyDesksFile` does five things: (1) merges the incoming desks list with any local desks the sender hadn't published yet (UNION rather than replace); (2) calls `reassignDeskIdByName` to unify a same-named local desk with an incoming one — both devices independently creating a "Personal" desk on first boot would otherwise carry two; (3) creates desk nodes for any incoming desks the local tree doesn't have, calling `absorbMatchingFolder` to fold up "ghost" folders the cursor delta may have built before the desks.json payload arrived (`<DeskName>/Inbox/Doc.md` events arriving before `.hush/desks.json` build a plain `<DeskName>` folder via `insertDocumentNode`; absorption walks every level for a same-named non-special folder/project and merges its children into the new desk — the gate used to require an `Inbox` sub-child but a Mac-side desk with a project at its root has no Inbox, so the gate now matches on name + non-special only); (4) calls `reconcileDesksWithStrayFolders` to do the same sweep for already-existing desks (catches the case where the user restarted and `ensureDesksTreeSpecials` already folded the ghost folder into another desk's children); (5) merges `desksMeta` entry-by-entry rather than replacing wholesale, so a payload from one device can't blow away saved choices for desks the sender hasn't touched. The per-desk last-file slot is wire-translated by `metaToWire` on serialize and resolved on apply: `lastFileId` (a device-local UUID) becomes `lastFileRemoteId` (Dropbox's stable `id`) via `get_sync_file_info`, and the receiving side maps it back via `find_synced_file_by_remote_id`; files without a remote_id (Local Sync, never-synced) ride as null and the receiving device falls back to the desk's first inbox item on a switch. The apply branch only overrides the local `lastFileId` when the resolution succeeds, so a peer with no syncable choice can't blow away the resolvable choice on this device. Dropbox layout migration (`sync/desks-migration.js`) runs once on the device that performs the boot-time always-on wrap: it enumerates `synced_files`, enqueues `rename` ops through the op log to prefix every path with `<DeskName>/`, and updates `relative_path` via `rename_sync_file`. Receiving devices don't enqueue any Dropbox writes — the cursor delta picks up the source's moves naturally.
...
**Meta bootstrap (`pushMetaIfAbsent`).** `reconcileSync` fires once per session on the first poll cycle. It calls `dbx.getMetadata` for `.hush/desks.json`, `.hush/projects.json`, and `.hush/styles.json`; if Dropbox already has the file, it skips its own push. The publisher (first device to activate) uploads its meta when nothing exists; the consumer (second device) skips and lets the cursor seed apply the publisher's payload. Without this guard the consumer's `[Personal]`-only `desks.json` would overwrite Mac's `[Personal, Mac's Desk]` payload before the cursor seed could pull it, the consumer's own write would `isOurRev`-suppress on read-back, `applyDesksFile` would never run with the publisher's content, and the publisher's desks would arrive on the consumer as orphan top-level folders. `applyDesksFile` also re-publishes the merged list when `desks.length > incomingDesks.length` (the consumer had local-only desks the publisher hadn't seen) so simultaneous-activation races converge to the union.

**Push gates.** Three layers of "don't push unchanged content" sit on every outbound write so a no-op autosave can't mint a fresh Dropbox rev that other devices would then pull back: (1) `executeUpload` in `op-log.js` SHA-256s the local content and compares against `SyncedFileInfo.lastSyncedHash`, returning early on equality. (2) `enqueueMetaUpload` in `meta-sync.js` keeps a per-filename hash of the last enqueued payload (`panes.json`, `projects.json`, `styles.json`, `desks.json`) and drops identical re-enqueues. (3) The hashing helper `sha256Hex` matches the Rust `SyncedFileInfo.last_synced_hash` format (lowercase hex over UTF-8 bytes) so the two sides compare directly without an extra Tauri round-trip.
