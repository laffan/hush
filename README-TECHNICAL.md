...
**Background layers** (`background-layers/`) follow the same lazy-import discipline. A style's `backgroundLayers` array (index 0 = back) mounts as a `#background-layers-host` div inserted as the editor element's first child, painted with `background: inherit` so per-layer `mix-blend-mode` composites against the real editor colour instead of a transparent backdrop (the host's `z-index: 0` makes it the isolation boundary; the scroller sits above at `z-index: 1`). Legacy shapes never rewrite themselves at apply time: `styles-panel-shared.js#resolveBackgroundLayersList` derives layers from the old single `backgroundImage` and from a WebGL2 `shaderLayer` on the fly, and `migrateStyle` bakes the conversion onto the style only when it's opened for editing. Caret effects sample `EditorView.coordsAtPos` through an event-driven, rAF-coalesced tracker (`caret-tracker.js`, refcounted singleton) and hand shaders a short trail of `(position, birth, seed)` uniforms so particles are pure functions of age — the draw loop parks itself once the trail fades and wakes on a 4 Hz caret poll, so an idle editor pays no per-frame cost. The style modal previews layers scoped to its preview pane under a `setScopedPreviewLock` that makes editor-context applies no-ops until the modal closes (the modal's own live saves emit `style-changed`, which would otherwise steal the layers back to fullscreen mid-edit).
