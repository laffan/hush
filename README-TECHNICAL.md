...
- **`cmd-button.js`** — iOS-only Touch mode. Mounts four floating pills in the bottom-left (⌘ hold-for-Cmd, ☰ palette, 📋 paste, ↶ undo) when `settings.touchMode` is on. On phone (`html.phone`) the ⌘ and ↶ pills are skipped — Cmd-modified flows and notebook undo aren't useful in that form factor — and CSS in `sidebar.css` re-anchors the remaining palette + paste pair to the centre of the bottom edge instead of stacking them on the left. Holding the ⌘ pill sets `window.__hushCmdHeld = true`, adds `body.cmd-held`, and dispatches a synthetic `Meta` keydown so listeners that already gate on `e.metaKey` (link-decorator's `_modifierHeld`, the `body.cmd-held` class flip, etc.) pick up the virtual modifier without per-call-site changes. Exports `isCmdHeld(event)` — call sites that previously checked `e.metaKey || e.ctrlKey` on pointer / drag events (text-drag, image-decorator, sortable-list drag-out, notebook drop / merge / link-click) consult this helper so the on-screen ⌘ counts as Cmd-held even though the touch event itself has `metaKey: false`. iPad detection lowered the touchpoints threshold to `> 0` (some WKWebView versions only expose 1 touch even though the device handles multi-touch)
...
**Session-state persistence.** Two per-window settings (`sidebarOpenPanel: "files" | "styles" | "versions" | null` and `sidebarPinned: bool`) ride through to the next session via `AppSettings` on the Rust side (`sidebar_open_panel: Option<String>`, `sidebar_pinned: bool`, both `#[serde(default)]`). `createSidebar()` runs a replay block at the end of init — guarded by a `_suppressStatePersist` flag so opening the persisted panel doesn't itself trigger a redundant save — that calls `openFilesPanel()` / `showPanel("styles")` / `openVersionsPanel()` based on the saved value and re-applies the pin class on narrow viewports. Subsequent open / close / pin toggles route through a single `persistSidebarState()` helper, including the `hide-panel` and `show-files-panel` event listeners that the `Cmd+\` shortcut fires (these were previously inlining cleanup that skipped the save, so closing via the keyboard left the on-disk state stuck at whatever was last opened by clicking).
