...
There is a floor on how thin a mark can usefully be, and it is not zero: a gaussian narrowed past about 0.5 CSS px of sigma spends all its brightness on the falloff, and once the supersampled buffer is downsampled the shape dissolves into a dotted line (bubbles) or vanishes outright (sparks). Small marks are therefore drawn as **hard shapes with a one-pixel `smoothstep` edge** rather than as shrunken gaussians — a disc for the spark motes, a ring for the bubbles — which holds full alpha in the middle at a 1-2px width. Gaussians are still right for anything wide enough to carry one (the ripple front, the underline's glow).

Each preset clamps its **coverage** to `[0,1]` and *then* multiplies by intensity. Clamping the product instead lets a preset whose coverage exceeds 1 (the HUD's ticks reach ~1.15) saturate against the alpha ceiling, and the top quarter of the intensity slider does nothing at all. Intensity is the effect's own brightness; the layer's per-appearance opacity is separate and applies to the slot element, exactly as it does for image and gradient layers.
