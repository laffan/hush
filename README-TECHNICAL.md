...
│   ├── api.js                         (Drive media-upload + export wrappers)
...
- **`api.js`** — thin Drive API wrappers. Phase 2 uses Drive (not Docs API directly) because Drive's media-upload converter handles HTML → Google Docs structural conversion server-side: push runs `PATCH /upload/drive/v3/files/{id}?uploadType=media` with `Content-Type: text/html`, pull runs `GET /drive/v3/files/{id}/export?mimeType=text/html`. `createDocumentFromHtml(title, html)` for Branch A of the link flow ("Create new") uses a multipart upload that bundles JSON metadata (`{ name, mimeType: application/vnd.google-apps.document }`) with the HTML body so Drive does the conversion in a single call. `listDocuments` filters Drive to `mimeType='application/vnd.google-apps.document'` for the picker. `viewUrl(docId)` synthesises the `https://docs.google.com/document/d/{id}/edit` URL for the link bar's title chip.
