...
- **`UIApplicationSupportsMultipleScenes` also opts the app into external-display sessions.** Plugging a monitor into an iPad handed Hush a second scene, which booted the app and restored the last file onto the monitor; the role is non-interactive, so the pointer couldn't reach it, nothing in the app could close it, and unplugging the cable was the only way out. `tauri-plugin-scene-reuse` refuses any scene whose session role contains `ExternalDisplay` (`requestSceneSessionDestruction`, windows hidden first so the monitor doesn't flash the document), which drops iPadOS back to mirroring. Application-role scenes are untouched, so a window the user drags to a monitor under Stage Manager still works.
