...
The Sync settings tab is split into three sub-tabs (**Dropbox Sync** / **Google Sync** / **Local Sync**) so each provider's chrome stays focused. The active sub-tab is module-local in `settings/settings-tabs-sync.js` (a `_activeSubTab` variable plus `getSyncSubTab` / `setSyncSubTab` exports). The outer `renderSyncTab(settings)` paints a 3-button nav at the top of the panel and delegates the body to whichever sub-renderer is active (`renderDropboxSubTab`, `renderGoogleSubTab`, `renderLocalSyncSubTab`). Bindings live in `settings/settings-sync-tab.js`: a single `bindSyncTab` wires the sub-tab nav clicks (which call `setSyncSubTab(id)` then `render()`) plus the existing Dropbox + Local Sync handlers, and a sibling `bindGoogleSubTab` wires Google's credential-form / connect / disconnect / test-connection / clear-log buttons.
