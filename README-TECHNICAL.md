...
- **`link-bar.js`** — the slim chrome pinned to the top of the editor column whenever the active doc has a link in `settings.googleDocLinks`. `initLinkBar(state)` is called once from `main.js`; it subscribes to `file-opened`, `settings-changed`, and `layout-changed` and re-renders / re-syncs on each. The bar hides while a notebook or project is open (neither maps to a single GDoc). **Mounted at body level** with `position: fixed; z-index: 90` rather than inside `#editor-container`, because the macOS Tauri `#drag-region` element (`position: fixed; z-index: 80; -webkit-app-region: drag`) would otherwise sit on top of the bar and steal every click for window-dragging. Lifting the bar above z-index 80 at the body stacking level is the only way to outrank it; a child of `#editor-container` (a `z-index: 0` stacking context) can never escape its parent's rank at the body level no matter how high its local z-index. `-webkit-app-region: no-drag` stays on the bar as belt-and-suspenders. `syncLayout()` copies `.cm-scroller`'s viewport rect into the bar's `left` / `right` offsets and copies its computed `padding-left` / `padding-right` so the chip + buttons land flush with the first and last columns of editor text, even when panels open / close or the column is resized. Title chip on the left opens the GDoc in the system browser via `@tauri-apps/plugin-opener`; three plain-text buttons on the right (Pull / Push / Unlink) fire immediately on click — no `window.confirm` prompts since both sides have version history and the inline status line is the synchronous feedback channel. **`setStatus(text, mode, autoClearMs)`** drives a `<span class="gdoc-link-bar-status">` between the chip and the actions: `Sending…` / `Receiving…` / `Unlinking…` for in-flight (italic, 0.75 opacity), `Sent ✓` / `Received ✓` in green for success (auto-clears after 2 s), `Failed: …` in red for errors (auto-clears after 4–5 s).
