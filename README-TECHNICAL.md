...
- **`link-bar.js`** — the floating bar pinned at the top of the editor (above `#editor-container`) whenever the active doc has a link in `settings.googleDocLinks`. `initLinkBar(state)` is called once from `main.js`; it subscribes to `file-opened` and `settings-changed` and re-renders on both. The bar hides while a notebook or project is open (neither maps to a single GDoc). Title chip on the left opens the GDoc in the system browser via `@tauri-apps/plugin-opener`; three buttons on the right (Pull / Push / Unlink) each prompt for confirmation via plain `window.confirm` (matches Hush's existing pattern, sits cleanly above any pane chrome). `setBusy(true)` adds `.gdoc-link-bar-busy` and disables all three during an in-flight API call.
