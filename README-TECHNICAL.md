...
- **Create Proofread Notebook** (`pdf/pdf-proofread.js`) produces an ordinary `.hushnote` with an extra `proof` envelope field; nothing downstream branches on it except the page rail (a live minimap of the page layer plus a sketch of the annotations over it) and the shelf, which hides the page layer. The tree node carries a `proofread` flag purely so the sidebar can pick a different glyph — the real metadata lives in the notebook.
