...
│   ├── pdf-sync.js                   (.hush/pdf.json — PDF registry + background download)
...
Listing-mode overlay shown when `state.selectedDocIds.length >= 2`. Mounts a single `#multi-select-view` host into `#app` at startup; the `multi-select-active` body class flips visibility (and sets `visibility: hidden` on `#editor-container` / `#notebook-container` so they retain layout underneath — no remount when the selection clears). Each row reads as `Folder / Project / Filename` via an `ancestorBreadcrumb(state, nodeId)` walker that skips the desk wrapper and the Inbox / Trash / Images specials. Rows route through `state.openFile` / `state.openNotebook` based on the persisted `node.type`; the open call emits `file-opened` / `notebook-open`, which the view also listens for and clears the selection on (so any single-open path anywhere in the app implicitly exits multi-select mode). Batch **Flag / Unflag** (label flips when every member is already flagged) and **Delete** (prompts with the full filename list, then trashes via `deleteTreeNode` in sequence to avoid concurrent `saveFileTree()` races) cover the immediate batch actions. The view also listens for `files-changed` so a deletion or rename outside this flow filters stale fileIds out of `selectedDocIds`. Esc clears the selection from anywhere in the app.
