...
- `dockedLeftWidth` / `dockedRightWidth` — footprints of edge-docked panes; `editor/modes.js` subtracts them from the column's usable width so the editor narrows beside a dock instead of sliding under it.
...
**Pane Docking (`pane/pane-dock.js`).** Dragging a floating pane's title bar within ~50 px of the left or right edge surfaces a blue drop zone (overlay built in `pane-drag.js`); releasing inside it calls `dockPane(pane, edge)`. A docked pane is positioned *beside* the host surface rather than over it: `applyDockGeometry` pins it to the absolute container edge, fills the full height, and defaults the user-controlled width to half of `(container − sidebar − right-chrome)`. The opposite edge stays resizable (`pane-drag.js` allows only the one perpendicular handle while docked); resizing rewrites `dockUserSize` and re-flexes. `publishDockCssVars()` writes four custom properties on `documentElement` — `--pane-dock-left-width` / `--pane-dock-right-width` (footprints) and `--pane-dock-left-edge` / `--pane-dock-right-edge` (absolute viewport-x of the dock's inner edge, sidebar already included) — and dispatches a `pane-dock-changed` document event. Containers consume the vars purely in CSS: `#stack-container` and `#pdf-container` set `left: var(--pane-dock-left-edge)` + `right: var(--pane-dock-right-width)` so the whole surface (scrollbars included) narrows; `#right-panel-overlay` and the notebook shelf read `--pane-dock-right-width` so the editor's own right-side chrome slides inboard of a right-dock instead of hiding behind it. `editor/modes.js` subtracts `state.runtime.dockedLeft/RightWidth` from the column's usable width. The notebook listens for `pane-dock-changed` to mirror the widths into `DrawingState.dockedLeft/RightWidth` (toolbar centring + `pocketRightInset`). `installDockReflowListeners()` (a window `resize` listener plus `ResizeObserver`s on the sidebar / outline / shelf / annotation-shelf — the sidebar width rides a CSS var, so a plain MutationObserver wouldn't fire) re-flexes every docked pane as chrome opens / closes / resizes. `publishDockCssVars` skips `display:none` panes so a dock owned by a different context doesn't leak its width into the active one, and `onContextChange` / `closePane` re-publish so switching desks or closing a dock fully resets the vars. Dock state (`docked`, `dockEdge`, `dockUserSize`, `dockPrev`) round-trips through `pane-persistence.js`.
