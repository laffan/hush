...
Both providers ride the shared `hushwriter://` deep-link plumbing in `main.js`. The Tauri-side `lib.rs` deep-link handler now recognises *both* `hushwriter://auth/callback` (Dropbox) and `hushwriter://auth/google/callback` (Google), and the emitted `oauth-callback` event carries a `provider` field (`"dropbox"` or `"google"`) so the listening settings window can dispatch to the right OAuth completion path. `handleOAuthCode(state, invoke, code, provider)` in `style-application.js` takes that provider arg and dynamically imports either `sync/dropbox.js` or `google-docs/auth.js`, calling the matching `completeOAuthFlow`. Browser-dev paths (where the OAuth provider redirects to `oauth-callback.html` rather than a deep link) read the OAuth `state` parameter (`state=google` is set on the Google auth URL; Dropbox doesn't set one) so the HTML page can redirect to the correct deep-link URL. `sessionStorage.hush_oauth_provider` mirrors the dispatch hint at the JS layer for the in-Settings-window flow.
