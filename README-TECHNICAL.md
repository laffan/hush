...
│   ├── image-paste.js
│   ├── zen-focus.js
│   ├── tabs.js                        (`---Tab name---` parser + joinTabs)
...
│   ├── tabs-sync.js                   (`---Tab name---` ↔ real GDoc tabs)
...
│       ├── tab-marker.js              (renders `---Tab name---` as a pill — see "Google Docs Integration")
│       ├── private-mode.js
│       ├── project-view.js
│       ├── sticky-headers.js
│       ├── grammar-check.js
...
│   ├── longview.js
│   ├── longview-parser.js
│   ├── longview-tabs.js               (foldable `---Tab name---` sections in the outline)
...
- **`api.js`** — thin Drive + Docs API wrappers. Phase 2 uses Drive (not Docs API directly) for the **content** path because Drive's media-upload converter handles HTML → Google Docs structural conversion server-side: push runs `PATCH /upload/drive/v3/files/{id}?uploadType=media` with `Content-Type: text/html`, pull runs `GET /drive/v3/files/{id}/export?mimeType=text/html`. Tab support adds a thin Docs API layer alongside Drive: `listTabs(docId)` enumerates `tabProperties.tabId` + `title` + `index` for every top-level tab via `documents.get?fields=tabs(tabProperties...)`; `createTab` / `renameTab` / `deleteTab` are `batchUpdate` wrappers; `replaceTabPlainText(docId, tabId, text)` fetches the tab's body, deletes its content range, then inserts plain text at `index: 1` of that `tabId`. `exportAsHtml(docId, { tabId })` adds the `tabIds=<id>&includeTabsContent=true` query params when a tab is specified, so the tab-aware pull path can export each tab's HTML in isolation. `createDocumentFromHtml(title, html)` for Branch A of the link flow ("Create new") uses a multipart upload that bundles JSON metadata (`{ name, mimeType: application/vnd.google-apps.document }`) with the HTML body so Drive does the conversion in a single call. `listDocuments` filters Drive to `mimeType='application/vnd.google-apps.document'` for the picker. `viewUrl(docId)` synthesises the `https://docs.google.com/document/d/{id}/edit` URL for the link bar's title chip.
- **`tabs-sync.js`** — orchestrates the tab-aware push and pull on top of `api.js`. **Push** parses the markdown into ordered sections via `editor/tabs.js#parseTabs` (first section = root tab, each `---Name---` marker starts a new tab), media-uploads the root section's HTML so the root tab carries the full formatting Drive's HTML importer produces, then walks `listTabs` to reconcile non-root tabs slot-by-slot (create-and-insertText for new sections, rename + replaceTabPlainText for existing slots, deleteTab for leftovers). The Drive media upload deliberately runs before the Docs API reconciliation because Drive's upload-to-a-tabbed-doc only replaces the root tab — every other tab survives the call, so the reconciliation step has stable tab IDs to work with. **Pull** does the inverse: `listTabs` enumerates the structure, then each top-level tab is exported via `exportAsHtml(docId, { tabId })` and converted to markdown using the caller's `htmlToMarkdownSafe` (so the Drive-export CSS-class inliner from `link-command.js` is reused), and the per-tab markdown is rejoined via `editor/tabs.js#joinTabs` with `---Tab name---` markers between sections. Non-root tab formatting is plain-text-only by design — structured insertion into a specific `tabId` isn't a Drive media-upload primitive, and emitting `insertText` + `updateTextStyle` + `updateParagraphStyle` batchUpdate requests for every formatted run would balloon the surface area; the root tab carries the formatted content, the rest are plain.
