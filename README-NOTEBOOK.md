...
src/notebook/
  ...
  perf-hud.ts             On-canvas perf diagnostics overlay + tracer singleton, gated behind Settings > Debug > Performance HUD (see PANNING-FIX.md)
  ui/
...
- **Perf HUD** — when **Settings > Debug > Performance HUD** (`settings.debugPerfHud`, default off) is on, the bridge mounts the on-canvas diagnostics overlay (`perf-hud.ts`) on the main notebook; a `settings-changed` listener applies the toggle live, and both teardown paths unmount it. See PANNING-FIX.md for how to read its reports.
