...
Everything upstream of `turn()` — drag deltas, coast velocity, the release samples — is in *face* px, so `GAIN` and `DIRECTION` are applied in exactly one place. The release throw differences two samples of the running `facePos` total, which means the press must seed the sample buffer with the current `facePos` and not with zero: seeding zero made a short flick read as "the wheel travelled its whole lifetime's distance just now", signed by wherever in the document the reader happened to be, so catching a coast and nudging it could fling it off the other way.
