...
- **Mounting** — dynamically imports `NotesCanvas`, passes shortcut settings, loads shapes from the backing file. `main-modes.js` switches `#app` into `.notebook-mode` *before* awaiting the (async) mount so a large notebook never leaves the doc editor's "Start writing…" placeholder on screen while shapes decode. During the load `mountNotebook` mounts a "Loading Notebook…" overlay (`.notebook-loading-overlay`, themed background + indeterminate progress bar) — appended only after a ~150 ms threshold so small notebooks that mount instantly never flash it, and torn down once shapes are loaded and the first render is queued.
