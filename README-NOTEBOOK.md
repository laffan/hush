...
A single drag-area selection surfaces three container-scoped actions on the selection toolbar (`ui/selection-toolbar.ts`), sitting between the Colors picker and the Trash button: **Arrange as grid**, **Swap-reorder mode**, and **Ripple-reorder mode**.
...
- **Recognize handwriting** — shown when the selection contains any `DrawShape` or `ImageShape` (so a previously-rasterized stroke group stays recognizable; the runtime must be a Tauri build on an Apple platform). Strokes + images are rasterized *with* the theme background and the raster is tuned for the recognizer: an adaptive scale targets a ~1600 px long side (clamped 1–6×, hard-capped at 4096 px), a 24 px whitespace margin is padded around the ink, and dark-theme rasters are inverted to dark-on-light (Vision is noticeably weaker on light-on-dark). The raster goes to the recognition engine in **`src/recognition/handwriting.ts`** — deliberately outside `src/notebook/` so Docs can adopt the same engine later — which base64s the PNG into the `recognize_handwriting` Tauri command (`src-tauri/src/commands/handwriting.rs`). That runs Apple's on-device **Vision** framework via raw `objc2` message sends: `VNRecognizeTextRequest` at the "accurate" level (the one that handles handwriting), pinned to the newest recognizer revision the OS supports (the default tracks the linked SDK, which can select an older model), with language correction on and language auto-detection enabled where available (macOS 13+ / iOS 16+). Recognized lines come back joined with newlines and land as a TextShape 16 px beneath the ink. For now the ink is left in place — replacing it with the text is a planned follow-up. Non-Apple targets return a descriptive error and the toolbar hides the button there anyway.
- **Recognize handwriting (Google ML Kit, iPad only)** — a second engine kept beside the Vision one for comparison, on its own **G** toolbar button (shown when the selection contains strokes; a stroke recognizer can't read images). No raster: the `DrawShape` point sequences go straight to ML Kit's Digital Ink recognizer, shifted to the selection bbox origin with the bbox as the `MLKWritingArea`, and timestamps synthesized (~15 ms/point, 300 ms pen-lift gaps) since Hush doesn't record pen timing. The Rust command (`src-tauri/src/commands/handwriting_ink.rs`) reaches the MLK* Objective-C classes **dynamically** (`AnyClass::get`, completion blocks via `block2`) rather than linking a framework — the binary builds with or without the pod and errors descriptively without it. ML Kit ships via CocoaPods only, so `scripts/ios-add-mlkit-pod.mjs` (chained into `npm run ios:init`, or run standalone via `npm run ios:add-mlkit` when `gen/apple` already exists) inserts `pod 'GoogleMLKit/DigitalInkRecognition'` into the generated Podfile, raises its platform floor to iOS 15.5, and runs `pod install`. The per-language model (~20 MB) downloads on first use — the command kicks the download and polls up to 60 s before asking the user to retry. macOS is excluded end-to-end (Google doesn't ship ML Kit for it): the button is gated by `isInkRecognitionAvailable()` and the command errors on non-iOS targets.
