...
The wheel **scrolls**, on both axes; ⌘ (or a trackpad pinch, which WebKit reports as a ctrl-wheel) **zooms**; **shift** pins the pan to one axis, chosen by the first shift-held event and held until shift comes back up (deciding per event let a wobbling swipe flip axes halfway through, which is the drift the modifier exists to suppress). Deltas apply 1:1 like a doc scroller, with `deltaMode` line / page units converted so a notched wheel moves a sane amount. A horizontal-dominant scroll still bubbles when the canvas is `paneHosted` — a stack column or a pane has a host scroller that owns it — but the main canvas keeps it and pans. Gutter mode is unchanged: it redirects the wheel into the host doc's scroller and has no zoom to offer.
