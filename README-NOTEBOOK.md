...
- **Decoded bitmaps** — fifty full-size pages decode to hundreds of MB, past what iPadOS gives a WKWebView. `image-budget.ts` keeps only images within ~1.5 screens of the viewport decoded once a notebook passes 12 image shapes, evicting the rest (the `dataUrl` never leaves the shape, so scrolling back re-decodes; a proof draws its small rail raster in the gap — see the proxy tier below). It recomputes on shapes-array identity — a split drag moves pages without changing their count — and on half-a-screen of camera travel, measured in **world** px: `camera.x/y` is a screen-space offset that pivot zoom scales along with everything else, so twenty pages down a proof a 2 % pinch step moved it by more than a screen and a fast pinch recomputed (and evicted, and re-decoded) on all sixty frames. Zoom itself is gated on a ratio, not on inequality, for the same reason. The page rail therefore gets its **own** small rasters baked at import time; a rail of fifty full-page `<img>`s would reintroduce exactly the problem the budget removes.
  ...
  The other half of what the reader felt was the **grey card**. Three sharp pages is all a fifty-page proof gets, and the fourth painted as the renderer's "broken image" placeholder — so scrolling a proof *flickered*, pages popping between paper and grey slab as the band slid over them, which reads as the canvas failing rather than as a budget doing its job. The budget therefore has a second tier: a proof already carries a 480 px thumbnail of every page (baked for the rail), a thumbnail is a **thirty-fourth** of a page, and `MAX_PROXY_MEGAPIXELS` buys twenty of them for what two pages cost — sized off the worst case rather than off ordinary reading, because splits can put fifteen pages on screen at zoom 0.25 and a soft tier that can't cover what is *visible* leaves exactly the holes it exists to fill. So the shape is a narrow band of sharp pages inside a wide one (`PROXY_MARGIN_SCREENS`) of soft ones, and `drawImageShape` falls through to the proxy whenever the page tier hasn't got the real thing — `crop` is in fractions of the source, so the same window indexes a thumbnail exactly as it indexes the page it was cut from, and a split piece stays the piece it is. Being able to fall back like this is also what lets the page tier stay as tight as it is without the tightness showing. Both tiers resolve a piece back to its page through `proof-pages.ts`, which is the rail's lookup rather than a second copy of it.

  Note where the budget actually *runs*: on the state-change pass (`_syncImageCache`), not the render pass. The render loop is dirty-driven and parks itself, so every deadline the controller sets needs a real timer behind it — which is what `_armBudgetPass` is. Scheduling a *frame* instead was a no-op twice over: the zoom-settle timer never refilled the read-ahead (a pinch left it switched off until some unrelated notify came along), and `_nudgeImageBudget` never took a second stack column's half of the shared ceiling off the first one, which is the case the sharing exists for.
