...
src/notebook/
  ...
  proof-pages.ts          Which proof page an image shape (or a cut piece of one) came from
...
- **Decoded bitmaps** — fifty full-size pages decode to hundreds of MB, past what iPadOS gives a WKWebView. Keep the whole page in view when tuning this, though: measured in a browser harness at an iPad-sized dpr-2 viewport, one open fifty-page proof held **502 MB**, of which the drawing layer's stage canvases were 335, the rail's fifty thumbnails 60, and everything below — the entire image budget — 89. The budget is the most *tunable* term, not the biggest one; `README-DRAWING`'s delta #39 and the pocket stash are what took the resting figure to 301 MB. `image-budget.ts` keeps only images within ~1.5 screens of the viewport decoded once a notebook passes 12 image shapes, evicting the rest (the `dataUrl` never leaves the shape, so scrolling back re-decodes; a proof draws its small rail raster in the gap — see the proxy tier below). It recomputes on shapes-array identity — a split drag moves pages without changing their count — and on half-a-screen of camera travel, measured in **world** px: `camera.x/y` is a screen-space offset that pivot zoom scales along with everything else, so twenty pages down a proof a 2 % pinch step moved it by more than a screen and a fast pinch recomputed (and evicted, and re-decoded) on all sixty frames. Zoom itself is gated on a ratio, not on inequality, for the same reason. The page rail therefore gets its **own** small rasters baked at import time; a rail of fifty full-page `<img>`s would reintroduce exactly the problem the budget removes.
  ...
  A ceiling alone bounds what a canvas *holds* and says nothing about what is *arriving*, which is the other half of the same memory and the half that reached the reader as a stall and then as a blank window. A flick down a fifty-page proof turns the keep set over faster than a 40 MB page decodes, so every pass started another one — for pages the camera had already gone past — and the resident peak became the ceiling plus whatever the gesture had stacked up, unbounded. `MAX_INFLIGHT_MEGAPIXELS` is the second bound: the keep set iterates in rank order, and only that much may be decoding at once, so the slot always goes to the nearest page still missing one. In megapixels rather than in images, for the reason the ceiling is — one proof page is fifty pasted screenshots, and 12 MP is one page at a time on iOS while a Desktop's thumbnails still go in a batch. `pdf-viewer-render.js` bounds its render queue the same way, and for the same symptom.

  The other half of what the reader felt was the **grey card**. Three sharp pages is all a fifty-page proof gets, and the fourth painted as the renderer's "broken image" placeholder — so scrolling a proof *flickered*, pages popping between paper and grey slab as the band slid over them, which reads as the canvas failing rather than as a budget doing its job. The budget therefore has a second tier: a proof already carries a 480 px thumbnail of every page (baked for the rail), and a thumbnail is a **thirty-fourth** of a page, so a slice of the ceiling too small to hold one more page holds a dozen of them. The shape is a narrow band of sharp pages inside a wide one (`PROXY_MARGIN_SCREENS`) of soft ones, and `drawImageShape` falls through to the proxy whenever the page tier hasn't got the real thing — `crop` is in fractions of the source, so the same window indexes a thumbnail exactly as it indexes the page it was cut from, and a split piece stays the piece it is. Being able to fall back like this is also what lets the page tier stay as tight as it is without the tightness showing. Both tiers resolve a piece back to its page through `proof-pages.ts`, which is the rail's lookup rather than a second copy of it.

  The soft tier is **reserved out of the share, never added to it** (`proxyReserve`), and it is **zero whenever more than one canvas is budgeted**. Both rules exist because the configuration that would most enjoy a soft tier — several proofs in a stack, where each column's share holds exactly one sharp page — is the one with no memory to spare: three columns is three drawing layers before a page decodes, and `MIN_DECODED_MEGAPIXELS` is a floor under each share, so the shares already sum past the global ceiling and anything per-canvas on top is memory the old policy never took. A stack therefore gets the page tier alone and exactly its previous footprint (measured: 121 MB across three fifty-page columns, before and after, to the byte). Reserving rather than taking the leftovers matters for the opposite reason: pages are costed at `ASSUMED_MEGAPIXELS` until they have been measured, so a scroll into fresh pages fills the share exactly and leaves no leftovers — the soft tier would vanish during scrolling, which is the only time it was needed.

  Note where the budget actually *runs*: on the state-change pass (`_syncImageCache`), not the render pass. The render loop is dirty-driven and parks itself, so every deadline the controller sets needs a real timer behind it — which is what `_armBudgetPass` is. Scheduling a *frame* instead was a no-op twice over: the zoom-settle timer never refilled the read-ahead (a pinch left it switched off until some unrelated notify came along), and `_nudgeImageBudget` never took a second stack column's half of the shared ceiling off the first one, which is the case the sharing exists for. The **box** needs an edge of its own for the same reason: a canvas can be built before it has one (a stack column mounts into a content area the stack has not laid out yet), the keep set is empty by definition while it measures zero, and the box arriving is not a state change — so without a `ResizeObserver` the column would evict everything on its first pass and then sit blank until the reader touched it.
...
The rail offsets from `state.rightInset` — the live canvas-right-edge → shelf-left-edge distance the canvas controller already re-measures per frame through the shelf's width transition — so it sits flush inboard of the shelf's grip when closed and is pushed left, not buried, when the shelf opens. Its left edge is a drag handle (60–300 px, persisted app-wide as `notebookProofRailWidth`); because the rail is right-anchored, dragging left makes it wider, so the resize delta is inverted. The handle **shows itself** — the toolbar drag tab's grip in its vertical form, the same 5x36 bar at the same 2 px radius, centred in the strip so the two handles a proof puts on screen read as one control in two places. Where it departs is the colour: the toolbar's grip is `currentColor` at low opacity drawn over canvas, while this one sits on the rail's own panel, so it takes the rail's border colour (`--proof-rail-border`, published from `theme.uiBorder`) and reads as part of the same edge. Hover and drag swap it to `currentColor`, which is the toolbar grip's resting language — lighting up is a step toward the handle you already know rather than a different idiom. Forms that hang the handle outside the rail were tried and dropped: half a pill over the canvas has to fight the rail's own corner clip, and it never read as better than the bar it replaced. An `ew-resize` cursor alone can't advertise a gesture: it only appears once the pointer is already on an 8 px strip nobody had a reason to try, so the rail's most useful move — widen it until the pages are legible — went unfound. The strip is exactly `RAIL_PAD` wide so it stays inside the scroller's padding and never eats a click meant for a page piece, and the drag puts `ew-resize` on `<body>` for its duration, since the pointer spends the gesture out over the canvas.
...
With no chart under the selection the same command takes the other reading: a selected text shape whose text is a markdown list is converted **in place** (`convertListShapesToOutlines`) — checkboxes on the items that lack them, the `outline` flag set, and the position shifted back by `OUTLINE_PAD` so the border is laid *around* the words rather than pushing them down and right. The gate is `isConvertibleList`, and it is strict on purpose: an outline paints its items and nothing else, so a shape mixing a paragraph with a list would have the paragraph vanish the moment the flag went on. A run that is already all checkboxes still converts — it is an outline in every respect but the flag, which is the case that covers a checklist dropped onto the canvas from a Doc.
