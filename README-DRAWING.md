...
src/notebook/drawing/
  ...
  drawing-layer-dom.ts   DOM scaffolding: transform wrapper, three stacked canvases,
                         the multiply-blended highlight wrapper + its bake pair
                         (delta #38), pocket-stash canvas, SVG overlay, helper
                         blit canvas. The highlight pair, the live / preview
                         overlays (delta #39) and the pocket stash all start
                         unbacked at 1x1 — see the deltas below.
...
The engine renders into a fixed-pixel canvas inside a CSS-transformed wrapper — pans/zooms are one GPU-composited transform, but the backing only covers `[origin, origin + worldSize]`. `re-anchor.ts` slides the origin (and grows `worldSize` at low zoom) so the viewport always lands inside the backing. `ensureCoverage(camera)` runs on every `setCamera`: a cheap predicate returns immediately with 10 % slack; otherwise a **same-size re-anchor** (the common case — pan at fixed zoom) translates stroke coords, slides the done canvas's pixels by the same delta, and repaints only the newly exposed edge strips (exact rects, not 512-px tiles, which would repaint most of the backing); origin deltas snap to the device-pixel grid so the blit never resamples. Size changes are hysteresis-gated so pinch drift can't force reallocation. Anchor state is one shared mutable record so a re-anchor propagates to every closure without re-binding. **The viewport it measures is the host box, not the window** (`getViewportSize`, fed the layer's container): a stack column or a floating pane covers a fraction of the screen, and `wantWorldSize` scales with 1 / zoom, so measuring the window made a proof zoomed out inside a 400 px column ask for a world size several times what the column could show — crossing the resize threshold, and paying a full rebake, for pixels nobody could see. The window is still the fallback while the container has no box of its own.
...
- **#37 straight-line capture** — `setStraightLine(bool)` makes a draw stroke keep only its anchor and the live point, so the drag lays down a ruled A→B line and the live preview is exactly what the release commits. It mutates `points[1]` in place, which is only sound because the active stroke is never streamline-cached (#26). Hush drives it per brush slot through `applySlotToEngine`, so switching brushes switches capture mode with everything else about the pen.

- **#38 highlight bake target** — highlighter strokes bake into a *second* canvas pair, in a second transform wrapper (`hlWrapper`) the host blends `mix-blend-mode: multiply` against the notebook canvas. The in-canvas `multiply` composite (`STROKE_MODES`) can only see ink already on the same canvas, so a highlight over a proofread page — an `ImageShape` painted one DOM layer down — came out as flat 50 %-alpha paint sitting on the words instead of tinted paper behind them. Note the blend lives on the *wrapper*, not on the canvas: the wrapper carries `will-change: transform`, which makes it a stacking context, and a blend inside a stacking context can only reach that group's own backdrop. Everything else about a highlighter stroke is unchanged — same stroke list, same tile index, same z-order, same erase, selection and export paths (the export already composited correctly, since it renders shapes and strokes into one flat ctx). `stroke-render.js` runs every clear / rebake / repaint / re-anchor shift per target, filtering strokes by `targetFor`. Whether a highlighter stroke blends is a **switch, not a rule** — `DrawingSlot.blend`, offered as the flyout's *Blend Stroke* checkbox whenever the slot's mode is Highlighter. Multiply is right over a printed page and wrong on a dark canvas, where it swallows light ink, so the default is per notebook rather than global: a slot the user has never switched resolves through `DrawingState.resolveSlotBlend` (on for a proofread notebook, off otherwise), and ticking the box makes the slot explicit so the notebook stops deciding. Each stroke records the resolved answer, so a notebook can hold both kinds; a stroke drawn before the switch existed carries nothing and follows the notebook default, which the host pushes in with `setHighlightBlendDefault` (changing it moves strokes between targets, so it repaints). The pair stays **unbacked (1×1)** and the wrapper unblended and unpromoted until a *blended* highlighter stroke actually exists, so a notebook that never reaches for the tool pays neither the two extra world-sized backings nor a per-frame backdrop read. The live canvas is reparented into the highlight wrapper while a highlighter slot is active (back *before the SVG overlay* on the way out, or the ink would paint over the eraser cursor and the selection chrome) so an in-progress stroke reads the same as the baked one. Two known edges: the drag *preview* canvas is not split, so a highlighter dragged in a mixed selection is unblended for the length of the gesture, and the pocket stash blits from the done canvas only, so a pocketed highlighter stroke doesn't render in the pocket tray.
...
- **#39 lazily-backed live / preview overlays** — the two overlays are world-sized canvases, **67 MB each at 4096²**, and neither holds anything except while a stroke is in flight (live) or a selection is mid-transform (preview). *Reading* a document fills neither. That matters because of what else is on the page: measured in a browser harness at an iPad-sized dpr-2 viewport, one fifty-page proofread notebook holds **502 MB**, of which the stage canvases are 335 MB and these two are 134 — while the whole viewport image budget everyone argues about is 89. So both start at 1×1 (same sentinel as #38's highlight pair) and are backed when drawing mode is entered, released when it is left, taking a proof at rest from 435 MB to 301.

  **The trigger is drawing mode, not the first stamp and not `setInputEnabled`.** Backing them on the first stamp puts a `canvas.width` reallocation — #27's IOSurface churn — *inside* the gesture; measured, that dropped the stroke outright on a third of attempts. And `setInputEnabled` also flips off for a transient pan (`isPanning`), so keying off pointer ownership would churn 134 MB twice per pan for anyone holding the pen. `DrawingLayer.setDrawingActive` is the separate signal, ordered after `setInputEnabled` so its cancel of an in-flight stroke always precedes a release. `ensureOverlayBacked` still runs at the two paint sites as a safety net — a no-op once backed, and a guarantee that no stamp can land on a 1×1 canvas. Every other site touching the overlays is a `clearCtx`, which on a 1×1 canvas is a correct no-op, which is why there are only two paint sites to guard.

  The **pocket stash** (`pocket-blit.ts`) got the same treatment and is not an engine delta: it mirrors the done canvas's pixel layout, costs the same 67 MB, and is only ever read for strokes the user has actually pocketed — none, on the notebook that can least afford it. It is backed by `ensurePocketStash` on the first stash and handed back by `releasePocketStash` when the re-anchor's restash finds nothing pocketed.
...
**Brush slots** (`state.brushSlots[0..3]`): `{ brushId, color, size, mode, blend, streamline, spacing, straightLine }`. Slots are per canvas and session-only — nothing about them reaches `AppSettings` — which is what lets `blend` default per notebook. `mode: "highlighter"` plus `blend` is the one pair that changes where a stroke is *baked* rather than only how it looks — see delta #38. Colors `"auto"` and `"heading"` are theme sentinels resolving at paint time (tagged on strokes for retint). Clicking the already-active slot opens its edit flyout; edits also retroactively restyle a live selection, with slider drags wrapped in one style session per gesture so a sweep is a single undo entry (`snapshotSelectedStyle` → `applyStyleToSelection` → `commitStyleHistory`). **`straightLine` is the one slot field with nothing retroactive about it** (delta #37): it governs how a stroke is *captured*, not how it looks, so `extendStroke` pins the active stroke to its anchor plus the live point and every other stroke keeps the shape it was already recorded with. Pressure is taken from the anchor rather than the moving end — a ruled line that tapers because the hand eased off mid-drag reads as a mistake. Erase and slice ignore the flag; both act along the path the pointer actually travelled. The toggle is offered twice, in the flyout's Line section and as the mini-palette's dot-in-circle square, from one shared glyph (`STRAIGHT_LINE_ICON`).
