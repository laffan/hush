...
The bridge is loaded asynchronously from `src/notebook/pencil-bridge.js`. On load, it sets `strokeEngine.setPencilOnly(true)` (delta #18) so finger contacts no longer start strokes — they fall through to whatever the notebook canvas does for an unmatched touch (i.e. nothing, leaving two-finger pan/pinch via the gesture recogniser as the way fingers interact with the drawing surface). The bridge is a no-op on macOS / Windows / Linux and on iOS builds where the native plugin failed to register.
