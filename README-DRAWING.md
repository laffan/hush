...
4. **Size-changed re-anchor** (zoom crossed `RESIZE_RATIO_THRESHOLD`): `translateAllStrokePoints` as before, resize the three stage canvases + pocket stash + wrapper + svg, and let the engine's `resize()` rebake repaint everything — still O(visible ink), but only zoom changes pay it.
